...
- ✅ **第二期 Agent Harness + Loop（已落地）**：`wenjia_agent/harness/`（`policy` + `loop`，Act→Verify→Revise→Finalize），包裹 SDK Runner，`runner.py` / `stream_runner.py` 已接线；前端运行流弹窗展示 `revise`/`verify` 节点。
- ✅ **A3 Guardrails（输出护栏，已落地）**：`wenjia_agent/guardrails/output_checks.py`——空回复、边界提醒、绝对化/恐吓措辞、八字一致性，纯函数可离线测试。
- ✅ **A3 Guardrails（输入护栏，已落地）**：`wenjia_agent/guardrails/input_checks.py`——越狱/提示词泄露、高风险决策、违法伤害类请求和超长输入会被确定性拦截，并写入 trace / Web 运行流。
- ✅ **A5 长期记忆（基础版已落地）**：`wenjia_agent/runtime/memory_store.py`——基于调用方 `client_id` 保存跨会话人物档案摘要；新会话会把长期记忆注入 Agent instructions，且提供 `list_long_term_memories_tool` 查询。
- ✅ **A6 报告事实校验（已落地）**：八字一致性校验联动 Verify–Revise，不一致触发修订。
- ⏳ 待办：B1 CI、C 领域工具、RAG/向量记忆、Docker/Postgres 迁移等。
...
| 记忆 | 按 session 的 SQLAlchemy 会话记忆 + `wenjia_profiles` 档案表 + 基于 `client_id` 的长期记忆表（sqlite，`create_all`） |
| 工具 | 排盘 / 五行 / 十神 / 神煞 / 城市 / 存档 / 列档案 |
| 评测 | 仅 mock 掉 LLM 的单测，**无智能体质量评测** |
| 可观测 | `tracing_disabled=True`，flow 事件仅供前端 UI |
| 护栏 | 确定性输入护栏 + 输出护栏，接入 runner / stream runner / trace |
...
**A5. 上下文 / 长期记忆管理 · P1**
- 现状：已具备 session 会话记忆、会话人物档案，以及基于调用方 `client_id` 的跨会话长期档案记忆。
- 目标：上下文窗口安全 + 更完整的跨会话长期记忆。
- 方案：基础版已做档案事实复用；后续补滚动摘要/裁剪、偏好抽取、可选向量记忆。
- 验收：超长对话不超限、不丢关键事实；跨会话能复用已保存人物档案。
...
新增 `wenjia_agent/harness/` 层，包裹现有 `Runner`：

```
wenjia_agent/harness/
...
| **P0–P1 / 第二期：Harness + Loop** | 落地 `wenjia_agent/harness/`，先做 Plan-first + Verify–Revise，串起护栏与事实校验 | 第 3 节 + A3 A6 |
